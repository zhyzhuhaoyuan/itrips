...
//			sb.append(user.getUserCode() + "-");
	...
	public String replaceToken(String agent, String token)
		...
		try {
			...
			System.out.println("tokenDetails[3]>>>>>>>>>" + tokenDetails[3]);
		...
		System.out.println("token有效期（剩余秒数 ）>>>>>>>>>>" + ttl);
		if (ttl > 0 || ttl == -1) {// 兼容手机与PC端的token在有效期
			newToken = this.generateToken(agent, user);
			// 4.缓存新token
			this.save(newToken, user);
			// 5.老的token延时过期,2分钟后旧token过期，注意手机端由永久有效变为2分钟（REPLACEMENT_DELAY默认值）后失效
